...
(function($) {
	...
	var $asideTab = $('.aside > li > a');
	$asideTab.on('click', function(e) {
		$asideTab.each(function() {
			$(this).parent().removeClass('active');
		});
		$(this).parent().addClass('active');
		...
		$(this).parent().parent().next('.tab-carousel-slide').children('.aside-container').hide();
		$(currentTab).show();
		e.preventDefault();
	});
	// $('.aside-container').hide();
	// $('.aside-container:first').show();
	$('.aside > li:first-child').addClass('active');
